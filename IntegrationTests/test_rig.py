...
if __name__ == "__main__":
    ...
    with dbtext.LocalMongo_DBText(testdbname, transactions=False) as db:
        ...
        command=[f"{texttest_home}/bin/Debug/net8.0/VendingMachineService.exe"]
    
        logging.info(f"starting VendingMachine on url {url}")
    
        process = Popen(command, stdout=PIPE, env=my_env)
        wait_for_message(process, "Content root path")
        try:
            if record:
                webbrowser.open_new(url + "/swagger/index.html")
